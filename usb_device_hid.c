...
extern const struct{uint8_t report[RPT_MOUSE_SIZE];}hid_rpt01;
extern const struct{uint8_t report[RPT_JOYSTICK_SIZE];}hid_rpt02;
extern const struct{uint8_t report[RPT_KEYBOARD_SIZE];}hid_rpt03;
extern const struct{uint8_t report[RPT_COM_SIZE];}hid_rpt04;
        ...
        switch(SetupPkt.bDescriptorType)
        {
            case DSC_HID: //HID Descriptor
                switch (SetupPkt.bIntfID)
                {
                    ...
                    case 1:
                        USBEP0SendROMPtr(
                            (const uint8_t*)&configDescriptor1 + 18 +9+7+9,		// magic number.  It is the offset from start of the configuration descriptor to the start of the HID descriptor.
                            sizeof(USB_HID_DSC)+3,
                            USB_EP0_INCLUDE_ZERO);
                        break;
                    case 2:
                        USBEP0SendROMPtr(
                            (const uint8_t*)&configDescriptor1 + 18 +9+7+9 +9+7+7+9,		// magic number.  It is the offset from start of the configuration descriptor to the start of the HID descriptor.
                            sizeof(USB_HID_DSC)+3,
                            USB_EP0_INCLUDE_ZERO);
                        break;
                    case 3:
                        USBEP0SendROMPtr(
                            (const uint8_t*)&configDescriptor1 + 18 +9+7+9 +9+7+7+9 +9+7+7+9,		// magic number.  It is the offset from start of the configuration descriptor to the start of the HID descriptor.
                            sizeof(USB_HID_DSC)+3,
                            USB_EP0_INCLUDE_ZERO);
                        break;
                }
